        ...
        form.validate({
			...
			submitHandler: function(e){
				...
				grecaptcha.ready(function() {
					grecaptcha.execute($(e).data('recaptcha'), { action: 'submit' })
					.then(function(token) {
						...
						button.value = token;
						html_form.append(button);

						$.ajax({
							type: "POST",
							url: '/include/send_data.php?action=small-form',
							dataType: "json",
							data: {name:name, phone:phone, url:url, domain:domain, subject:subject, form:form},
							success: function(data){
								...
								if(data.response){
									if(data.form == 1){
										...
										// window.location.href = "/order/thank-you-1.php?name="+data.name+"&email="+data.email+"&form="+data.form;
									}else{
										$(e).trigger("reset");
