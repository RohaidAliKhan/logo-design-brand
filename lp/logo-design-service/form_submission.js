...
$(document).ready(function () {
  $(".form_submission").each(function () {
    ...
    form.validate({
      ...
      submitHandler: function (e) {
        ...
        var package_name = $(e).find('[name="package_name"]').val();
        ...
        $.ajax({
          ...
          dataType: "json",
          data: { name: name, email: email, phone: phone, message: message, url: url, domain: domain, subject: subject, optional: obj, form: form, package_name: package_name },
          success: function (data) {
            if (data.response.status) {
              localStorage.setItem("crm_id", data.response.message);
              localStorage.setItem("crm_email", data.response.email);
              localStorage.setItem("crm_name", data.response.name);
              localStorage.setItem("crm_phone", data.response.phone);
              var package_category = localStorage.getItem("package_category");
              if (package_category == 1) {
                window.location.href = "../logo-brief/";
              } else {
  ...
  $("#briefForm").validate({
    ...
    submitHandler: function (e) {
      $(e).find(".loader").show();
      var logo_info = $(e).find('[name="logo_info"]').val();
      var selected_logo = $(e).find('[name="selected_logo"]').val();
      var brief_text = $(e).find('[name="brief_text"]').val();
      var brief_tagline = $(e).find('[name="brief_tagline"]').val();
      var brief_description = $(e).find('[name="brief_description"]').val();
      var design_concept = $(e).find('[name="design_concept"]').val();
      var existing_website = $(e).find('[name="existing_website"]').val();
      var client_id = localStorage.getItem("crm_id");
      var client_email = localStorage.getItem("crm_email");
      $.ajax({
        type: "POST",
        url: "/includes/send_data.php?action=logo_brief",
        dataType: "json",
        data: {
          logo_info: logo_info,
        ...
        success: function (data) {
          console.log(data);
          if (data.response.status) {
            window.location.href = "/lp/logo-design-service/payment/";
          } else {
            $(e).find(".error").html("Error Occurred");
          }
          $(e).find(".loader").hide();
